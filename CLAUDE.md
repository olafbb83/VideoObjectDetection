...
- **ESP32 віддає MJPEG-потік рівно одному клієнту.** `esp_http_server` крутить
  усі сокети в одній задачі, а обробник стріму блокується назавжди. Відкрита
  вкладка браузера з `http://esp32cam.local/` не дасть Python-клієнту підключитись.
  `/status` на порту 80 живий завжди — по полю `streaming` видно, чи зайнято.
