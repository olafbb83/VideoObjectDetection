...
def probe_status(stream_url: str, timeout: float = 3.0) -> dict | None:
    """
    Питає /status на порту 80 — він живий навіть тоді, коли з потоком проблеми
    (порт 81 обслуговує окремий httpd, щоб блокуючий обробник стріму не
    заморожував телеметрію).

    Поле `clients` показує, скільки глядачів зараз висить на потоці.
    Це важливо: камера обслуговує кількох клієнтів одночасно, але вони ДІЛЯТЬ
    спільну пропускну здатність — виміряно 13.3 + 7.4 fps на двох там, де один
    отримував 21. Відкрита вкладка браузера вдвічі ріже FPS у Python-клієнта.
    """
    try:
